...
public class UDCJDBCTemplate {
    ...
    private static final String STMT_QUERY_SESSION = "select count(*) from "
            + ServiceConfig.get("udc.table.session.name")
            + " where id=?";
	...
	private Map<Long, String> extractSessionProperties(List<UsageDataInfo> infos) {
        Map<Long, String> props = new HashMap<Long, String>();
        for (UsageDataInfo usageDataInfo : infos) {
            if (!props.containsKey(usageDataInfo.getSessionId())
                    && !StringUtils.isEmpty(usageDataInfo
                    .getSessionProperties())
                    && !existsInDB(usageDataInfo.getSessionId())) {
    ...
    private boolean existsInDB(long sessionId) {
        long value = jdbcTemplateObject.queryForLong(STMT_QUERY_SESSION, sessionId);
        return value != 0;
    }

    public int[] create(final List<UsageDataInfo> infos) {
        final Map<Long, String> props = extractSessionProperties(infos);
        final Long[] keys = (Long[]) props.keySet().toArray(new Long[0]);
        ...
        return jdbcTemplateObject.batchUpdate(STMT_INSERT,
            ...
            public void setValues(PreparedStatement ps, int i)
                ...
                ps.setLong(3, info.getSessionId());
